...
const audio = {
  ...
  lift: "src/audio-lift.ogg",
  home: "src/audio-home.ogg"
};
const cardWidth = 73;
const cardHeight = 98;
const state = {
...
const faceUpLastOnDesk = index => {
  ...
  if (card && !getCard(card).facingUp) {
    postMessage({ state: GameState.FlipDesk });
    faceUp(card);
  }
};
...
const moveCardTo = (dest, i, card) => {
  if (!started) {
    started = true;
    postMessage({ state: GameState.Start });
  }
  ...
  state[location][pile].cards = state[location][pile].cards.filter(
    elem => moving.indexOf(elem) === -1
  );

  // append to destination
  state[dest][i].cards = state[dest][i].cards.concat(moving);

  if (dest === "finish" && location !== "finish") {
    audio.home.play();
    postMessage({ state: GameState.MoveHome });
  }

  if (dest === "desk") {
    if (location === "finish") postMessage({ state: GameState.DrawHome });
    if (location === "deal") postMessage({ state: GameState.DrawDeal });
  }
};
...
const captureMove = index => event => {
  ...
  moving = setTimeout(() => {
    ...
    for (let i = 0, l = destinations.length; i < l; i++) {
      const { top, left, width, height } = destinations[
        i
...
const win = () => {
  ...
  function removeAnimation(event) {
    ...
    postMessage({ state: GameState.Finish });
  }
  document.addEventListener("click", removeAnimation, false);
};

const postMessage = data => {
  window.parent.postMessage({ game: "js-solitaire", GameState, ...data }, "*");
};
